...
class Forecast:
    ...
    def __call__(self, data: typing.Optional[typing.Sequence[typing.Sequence[Decimal]]], **kwargs) -> "Forecast":
    ...
    def weighted_moving_average(self, periods: int, weights: typing.Union[list, tuple]) -> "Forecast":
        ...
        if len(weights) != periods:
            raise Exception(f"Weights must have as many elements as periods. Weights: {len(weights)} Periods: {periods}.")
    ...
    def exponential_smoothing_with_trend_and_seasonality(self, season: int, periods: int, alpha: Decimal, beta: Decimal,
    ) -> "Forecast":
        if type(alpha) != Decimal:
            raise TypeError("alpha must be of type Decimal.")
        ...
        seasonality = [((data_n_minus_1 + self.data[-2][i]) / total_units) * Decimal(periods)
...
def linregress(x, y, alternative="two-sided"):
    ...
    """Calculate a linear least-squares regression for two sets of measurements.
       ...
       The implemention copies that of scipy.linregress.
